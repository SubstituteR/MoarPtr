...
namespace moar
{
	namespace v4
	{
		...
		/*template<concepts::Function T1, concepts::CommonType T2 = types::default_calling_convention, concepts::CommonType T3 = type_traits::select_default_t3_t<T2>>
		class detour_ptr_base;

		template<concepts::Function T1, concepts::CommonType T2 = types::default_calling_convention, concepts::CommonType T3 = type_traits::select_default_t3_t<T2>>
		class detour_ptr;

		template<typename RT, concepts::CommonType T2, concepts::CommonType T3, typename ...A>
		class detour_ptr_base<RT(A...), T2, T3> : public pointer_base<detour_ptr_base<function_signature_t<RT(A...), type_traits::select_calling_convention_t<T2, T3>, type_traits::select_variadic_t<T2, T3>>>>
			using base = pointer_base<detour_ptr_base<function_signature_t<RT(A...), type_traits::select_calling_convention_t<T2, T3>, type_traits::select_variadic_t<T2, T3>>>>;
			friend class base;
		...
		protected:
			typename base::pointer_type mutable_ptr_ = nullptr;
			detour_ptr_base() = default;
		public:
			[[nodiscard]] auto mut() { return reinterpret_cast<void**>(&this->mutable_ptr_); }

			[[nodiscard]] auto dirty() { return this->get() != this->mut(); }
		};

		template<typename RT, concepts::CallingConvention T2, typename ...A>
		class detour_ptr<RT(A...), T2, void> : public detour_ptr_base<RT(A...), T2, void>
		{
		public:
			RT operator()(A... args)
			{
		...
		template<typename RT, concepts::CallingConvention T2, typename ...A>
		class detour_ptr<RT(A...), T2, types::variadic_t> : public detour_ptr_base<RT(A...), T2, types::variadic_t>
		{
		public:
			template<typename ...B>
			...
			template<typename ...B>
			RT original(A... args, B... vargs)
			{
				return this->mutable_ptr_(args..., vargs...);
			}
		};
		...
		*/
		...
		> /* typename not declared yet. */
		{
			using base = pointer_base<
				function_ptr_base<RT(A...), type_traits::select_calling_convention_t<T2, T3>, type_traits::select_variadic_t<T2, T3>>,
				function_signature_t<RT(A...), type_traits::select_calling_convention_t<T2, T3>, type_traits::select_variadic_t<T2, T3>>
			>;
			friend class base;
